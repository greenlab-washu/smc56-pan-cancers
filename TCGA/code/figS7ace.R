...
cohort_lung <- bind_rows(cohort_sample) %>%
  filter(project %in% c('TCGA-LUSC','TCGA-LUAD') &
           str_detect(Tumor_Sample_Barcode,'-01A$')==T) %>% 
  mutate(Patient_Barcode=str_extract(Tumor_Sample_Barcode,'^.{12}')) 

luad_survival <- read.delim(paste(input_dir, '/cBioPortal/pan_can_atlas/luad_tcga_pan_can_atlas_2018/data_clinical_patient.txt',
                                  sep=''),
                            sep = '\t', header = T, comment.char = '#') %>%
  mutate_at(c('DAYS_LAST_FOLLOWUP','DAYS_TO_BIRTH','AJCC_STAGING_EDITION'), as.integer)
lusc_survival <- read.delim(paste(input_dir, '/cBioPortal/pan_can_atlas/lusc_tcga_pan_can_atlas_2018/data_clinical_patient.txt',
                                 sep=''),
                           sep = '\t', header = T, comment.char = '#') %>%
  mutate_at(c('DAYS_LAST_FOLLOWUP','DAYS_TO_BIRTH','AJCC_STAGING_EDITION'), as.integer)
cbioportal_clinical <- bind_rows(luad_survival,lusc_survival) %>%
  mutate(OS_STATUS_DIGIT=as.numeric(str_extract(OS_STATUS,'^.{1}'))) %>%
  filter(!is.na(OS_MONTHS))

luad_ploidy <- read.delim(paste(input_dir, '/cBioPortal/pan_can_atlas/luad_tcga_pan_can_atlas_2018/data_clinical_sample.txt',
                                  sep=''),
                            sep = '\t', header = T, comment.char = '#') 
lusc_ploidy <- read.delim(paste(input_dir, '/cBioPortal/pan_can_atlas/lusc_tcga_pan_can_atlas_2018/data_clinical_patient.txt',
                                  sep=''),
                            sep = '\t', header = T, comment.char = '#') 
cbioportal_ploidy <- bind_rows(luad_ploidy,lusc_ploidy)

## S7a - 3 groups ----
df <- left_join(cohort_lung, tmb_master, 
...
ggsave(paste(output_dir,'/TCGA/figures/lung_log10TMB_3groups.eps',sep=''),
...
df %>% select(all_of(c('Tumor_Sample_Barcode','TMB','COHORT'))) %>%
  ...
  stat_compare_means(comparisons = list(c('syn','nonsyn_nondel'), c('nonsyn_nondel','del'), c('syn','del'),
                     ...
                     method = 'wilcox', label = 'p.adjust')
ggsave(paste(output_dir,'/TCGA/figures/lung_log10TMB_6groups.eps',sep=''),
...
df_ploidy <- inner_join(df,cbioportal_ploidy,
  ...
  mutate(COHORT=fct_relevel(COHORT,'syn','del','nonsyn_nondel',
                            'low_cnv','high_cnv','wt')) %>%
  mutate(status=fct_relevel(status,'SmV','CNV','WT'))
# number of tumors without ANEUPLOIDY score
select(df_ploidy,all_of(c('Tumor_Sample_Barcode','ANEUPLOIDY_SCORE'))) %>%
  unique() %>%
  filter(is.na(ANEUPLOIDY_SCORE)) %>%
  nrow() #403
# so the number of tumors in each group are
filter(df_ploidy,!is.na(ANEUPLOIDY_SCORE)) %>%
...
select(df_ploidy,all_of(c('Tumor_Sample_Barcode','status','ANEUPLOIDY_SCORE'))) %>%
  ...
  theme(legend.position = 'none') +
  scale_color_manual(values=c('#A52A2A','#702963','#28446f')) +
  labs(caption="There are 403 tumors with missing aneuploidy score among 823 tumors included in this dataset.") +
  stat_compare_means(comparisons = list(c('SmV','CNV'),c('CNV','WT'),c('SmV','WT')),
                     method = 'wilcox',label='p.adjust') 
ggsave(paste(output_dir,'/TCGA/figures/lung_aneuploidy_3groups.eps',sep=''),
       dpi=320, width=3, height=3, units='in')