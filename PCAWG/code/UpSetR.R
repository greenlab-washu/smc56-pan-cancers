...
x1 <- select(smc56_snv, all_of(c('Hugo_Symbol', 'Tumor_Sample_Barcode'))) %>%
...
# this was saved as 'upset_smc56_alterations.eps'
## different types of alterations ----
# each tumor + type of alterations counted only once 
df <- rbind(x1, x2, x3) %>%
  select(all_of(c('Tumor_Sample_Barcode', 'alteration_type'))) %>%
  unique() %>%
  as.data.frame() %>%
  mutate_at(c('Tumor_Sample_Barcode', 'alteration_type'), as.character) %>%
  table() %>%
  as.data.frame() %>%
  pivot_wider(names_from = alteration_type, values_from = Freq) %>%
  as.data.frame()
# plot the UpSet plot
upset(df, sets = c('High CNV', 'Low CNV', 'SmV'), keep.order = TRUE,
      mainbar.y.label = "Number of tumors", sets.x.label = "Number of tumors",
      order.by = 'freq', nintersects = 50, mb.ratio = c(0.6, 0.4),
      shade.alpha = 1, matrix.dot.alpha = 1
)
...
# this was saved as 'no_intron_upset_smc56_alterations.eps'
# SmV only 
x1 %>% 
  select(all_of(c('Gene_Symbol', 'Tumor_Sample_Barcode'))) %>%
  unique() %>%
  as.data.frame() %>%
  mutate(Gene_Symbol = with(., ifelse(Gene_Symbol=='ANKRD32',
                                      'SLF1', ifelse(Gene_Symbol=='FAM178A',
  ...
  pivot_wider(names_from = Gene_Symbol, values_from = Freq) %>%
  as.data.frame() %>%
  upset(sets = c('SLF2','SLF1','NSMCE4B','NSMCE4A','NSMCE3',
                   'NSMCE2','NSMCE1','SMC6','SMC5'), keep.order = TRUE,
      mainbar.y.label = "Number of tumors", sets.x.label = "Number of tumors",
      order.by = 'freq', nintersects = 50, mb.ratio = c(0.6, 0.4),
      shade.alpha = 1, matrix.dot.alpha = 1
)
# this was saved as "no_intron_upset_smc56_smv.eps"