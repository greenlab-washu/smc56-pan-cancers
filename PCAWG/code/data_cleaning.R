...
## the aliquot_id (sample_sheet) == Tumor_Sample_Barcode (raw_snv) == colnames(raw_cnv)
## the icgc_donor_id (sample_sheet) == Donor_ID (raw_snv) == PATIENT_ID (raw_clinical) == PATIENT_ID (raw_clinical2)
## the icgc_specimen_id (sample_sheet) == SAMPLE_ID (raw_clinical)
## the icgc_sample_id (sample_sheet) == ICGC_SAMPLE_ID (raw_clinical)
# after matching the columns between different data tables 
# I should decrease the size of this sample_sheet and give it to Ilham 
...
## the number of tumors/patients with ----
# SmV, CNV, and clinical information available 
...
master_tmb <- select(raw_snv, all_of(c('Tumor_Sample_Barcode', 'Hugo_Symbol',
  ...
  mutate(TMB = abs_mut_number/2800) # this is new adjustment as i am working with WGS
...
write.csv(master_tmb, paste(output_dir, '/data/white_list_tmb.csv', sep = ''),
          row.names = FALSE)
## trinucleotide context of all tumors
## recurrent SMC5/6 mutations
## define tumors IDs in each cohort